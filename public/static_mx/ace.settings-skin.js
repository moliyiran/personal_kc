...
(function($ , undefined) {
    ...
    $('#skin-colorpicker+div').addClass("btn");
    $('#skin-colorpicker+div>a.dropdown-toggle').css({ color: "white",display:"block" });
    $('#skin-colorpicker+div>a.dropdown-toggle').html('<i class="ace-icon fa fa-pencil"></i>');
    ...
    //$("a.colorpick-btn", '#skinselect').on('click', function () {
    //    var skin_class = $(this).data('skin');
    //    setskin(skin_class)
    //});

    var setskin = function (skin_class) {

        $("#skin-colorpicker+div a.colorpick-btn").removeClass("selected");
        var color = $('#skin-colorpicker').find('option[data-skin="' + skin_class + '"]').val();
        $("#skin-colorpicker+div a.colorpick-btn[data-color='" + color + "']").addClass("selected");
        //$('#skin-colorpicker').val();
    ...
    function applyChanges(skin_class) {
       ...
       // $('.sidebar[data-sidebar-scroll=true]').ace_sidebar_scroll('reset')
        ...
        //特殊处理左侧菜单收缩按钮(修改模板)
        $("#sidebar-collapse").addClass(" sidebar-collapse");
        //特殊处理左侧菜单皮肤选择
        $('#skin-colorpicker+div').addClass("dropdown dropdown-colorpicker");
    };


    var skin = ace.data.get("skin");
    if (skin) {
        setskin(skin);
    } else {
        setskin("no-skin");
